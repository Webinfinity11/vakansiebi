...
Migration `032_scraper_limits.sql` adds admin-managed per-run limits, globally or per source: total detail attempts (1–200), discovery/detail time (1–8 minutes), additional listing/sitemap requests (0–3), and separately requested description repairs (0–20, at most 3 additional minutes). Zero additional pages keeps the first listing page; zero repairs pauses that queue. The worker uses the lower of saved limits and the runner's existing ceilings. Settings take effect on the next run without a deployment; in-flight work finishes normally. Limits are per run, not a daily financial cap. Smaller batches retain unfinished work, but scanning fewer listing pages may miss short-lived/new postings on busy sources. The explicit saver preset sets 100 details / 4 minutes / 1 additional page / 5 repairs and economical mode; the migration preserves existing runner limits until an admin changes them. Apply migration 032 before publishing the API/worker.

Migration `031_scraper_economy.sql` enables economical processing. Admins can switch all sources or each source between economical and full modes. Economical mode spreads routine published-record verification over a three-day target, with a 10% batch floor and additional capacity for records unverified for five days; existing new-item priority, source rate limits, the run time budget and seven-day verification expiry remain in force. Large queues or missed runs can delay the target. Full mode uses otherwise idle detail capacity for rechecks. This is deliberately not a new-only mode: withdrawn and expired vacancies still need verification. Apply the migration before deploying the API and worker.
...
Search Console-ის მთავარი sitemap: `https://jobx.ge/sitemap.xml`; `robots.txt` ამ მისამართს უთითებს. ერთი `<urlset>` შეიცავს ძირითად გვერდებს, ინდექსირებად ძიებებს, კომპანიებსა და საჯარო აქტიური ვაკანსიების კანონიკურ მისამართებს. ვაკანსიების სექცია ინდექსშიც არის დაკავშირებული, რათა Google-მა განცხადებები უშუალოდ აღმოაჩინოს.

XML მოთხოვნისას იქმნება და CDN-ზე 1 საათით ინახება; ახალი build მონაცემების განახლებისთვის საჭირო არ არის. პასუხი ნაკადურად გაიცემა. 50,000 URL-ის ან 50 MiB-ის გადაჭარბებისას ბრუნდება სექციების ინდექსი. ძველი `/sitemap-index.xml` შენარჩუნებულია და შეიცავს `/sitemap-pages.xml`, `/sitemap-searches.xml`, `/vacancies/sitemap.xml` და `/companies/sitemap.xml` სექციებს. მთავარი სიის შექმნის შეცდომაზე ბრუნდება სექციების ინდექსი HTTP 200-ით და `Cache-Control: no-store`-ით, რათა მომდევნო მოთხოვნამ სრული სიის შექმნა ხელახლა სცადოს. სექციების მონაცემთა შეცდომაზე კვლავ ბრუნდება არაკეშირებადი 503; სტატიკური ინდექსი ბაზის გარეშეც მუშაობს.
