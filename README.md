...
The cloud scraper runs in GitHub Actions (`.github/workflows/scrape.yml`) every 3 hours from 07:17 to 22:17 Tbilisi time (none at night). A first `plan` job asks the database once which sources are due and starts runners only for those. A source whose checks keep finding nothing waits longer: the second empty check in a row doubles its interval and the third doubles it again, up to 12 hours; anything new resets it (`quietIntervalMinutes` in `worker/next-run.ts`). Each source respects its admin interval, enabled status and manual request. GitHub schedules can be delayed. The two government sources (hrgov and worknet) use `scripts/local-gov-sources.sh` on this Mac, also every 3 hours; the Mac must remain awake.
...
The submission route starts delivery with Next.js `after()`. An atomic two-minute lease prevents concurrent sends. Retries reuse the unchanged payload and `jobx-invoice/<invoice UUID>` idempotency key. The Vercel cron (`/api/cron/invoice-email`, hourly at :17 from 07:17 to 23:17 Tbilisi time, up to five queued messages) recovers deferred work. It shares its minute with the scraper and the three-hourly sitemap warmers so the database wakes once for all of them, and none of them runs at night: Neon bills the minutes it is awake, and a ten-minute cron alone kept it from ever scaling to zero. If moving to Hobby, use an external scheduler or adjust the schedule to that plan's limit. Immediate submission retries can also recover due messages. Failed calls back off and stop after six attempts; permanent provider rejection stops immediately. `sent` means Resend accepted the message, not that it reached the inbox: inspect Resend delivery events for bounces.
