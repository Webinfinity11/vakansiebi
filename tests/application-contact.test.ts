...
void test('the default mail draft keeps the editable signature placeholder', () => {
  const draft = new URL(
    emailDraft('jobs@example.com', 'Developer', defaultApplicationBody),
  );
  assert.equal(draft.searchParams.get('body'), defaultApplicationBody);
  assert.ok(draft.searchParams.get('body')!.includes('[შენი სახელი]'));
});
