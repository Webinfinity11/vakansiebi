/* Hybrid dark theme: public palette decisions live in app/tokens.css.
   This generator keeps the existing selector specificity and source order for compatibility,
   referring to those tokens instead of recalculating their colors. Only remaining one-off
   literals use the legacy transform. shadcn and printable/logo exceptions retain their rules.
   Run `npx tsx scripts/build-dark-theme.ts` after changing public styles or tokens. */
...
/* Resolve public tokens only for legacy color detection. The emitted CSS retains var()
   references so editing a palette value never requires editing component declarations. */
export function publicTokens() {
  const light = new Map<string, string>();
  const dark = new Map<string, string>();
  for (const block of readBlocks(
    readFileSync('app/tokens.css', 'utf8').replace(/\/\*[\s\S]*?\*\//g, ''),
  )) {
    const target = block.selector === ':root' ? light : dark;
  ...
  // Only light values are needed for detection; aliases keep their live CSS relationship.
  const primitives = new Map<string, string>();
  for (const block of readBlocks(readFileSync('app/globals.css', 'utf8'))) {
  ...
  for (const [name, value] of light) {
    ...
    if (alias) {
      const literal = primitives.get(alias[1]);
      if (literal === undefined)
        throw new Error(`Undefined primitive alias: ${alias[1]}`);
      light.set(name, literal);
    }
  }
  return { light, dark };
}
const publicTokenPattern = /var\((--jobx-[a-z0-9-]+)\)/g;

function build() {
  const { light } = publicTokens();
  const resolve = (value: string) =>
    value.replace(publicTokenPattern, (_, name: string) => {
      const literal = light.get(name);
      if (literal === undefined)
        throw new Error(`Undefined public token: ${name}`);
      return literal;
    });
  const out: string[] = [
    '/* Generated by scripts/build-dark-theme.ts — do not edit by hand.',
    '   Palette overrides live in tokens.css. These compatibility rules preserve the cascade;',
    '   only remaining literal colors use the legacy transform. */',
    '',
    ...
    for (const block of blocks) {
      ...
      for (const declaration of block.declarations) {
        ...
        if (masthead && !/^(background|background-color)$/.test(property))
          continue;
        if (masthead && !/#fff\b|#ffffff\b|\bwhite\b/i.test(resolved)) continue;
        if (!hexPattern.test(resolved)) {
