...
Neither government host is in the GitHub matrix: vacancy.hr.gov.ge times out from every runner (2026-09-09) and so does worknet.moh.gov.ge (2026-09-12, run 34701274010, `UND_ERR_CONNECT_TIMEOUT`). Myjobs.ge does work from the cloud: its first cloud run imported 326 records and emptied its queue. `scripts/install-local-gov-sources.sh` registers a launchd agent on this Mac that runs hrgov and worknet every 30 minutes when due; advisory locks keep it from overlapping with the workflow.
