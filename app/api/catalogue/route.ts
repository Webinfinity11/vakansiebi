...
export async function GET() {
  try {
    ...
    const { rows } = await db().query(
      `${plan.cte}, visible AS MATERIALIZED (SELECT j.id,j.p_category AS category,j.group_key FROM searchable j WHERE ${plan.where})
    SELECT (SELECT count(*)::int FROM visible) total,
      COALESCE((SELECT jsonb_agg(c) FROM (SELECT category name,count(*)::int count FROM visible GROUP BY 1 ORDER BY 2 DESC) c),'[]'::jsonb) categories,
